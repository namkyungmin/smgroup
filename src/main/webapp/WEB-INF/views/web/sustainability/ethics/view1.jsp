...
<body>
        ...
        <div id="container">
            <div id="contents">
				...
				<div class="sec2 section">
					<div class="sec_inner1 sec_inner">
						...
						<div class="sec_txt">
							...
							<p class="txt2">윤리교육은 윤리경영에 있어 가장 기본이며 필수적 요소입니다. <br class="b">윤리적 문제에 부딪혔을 때 어떻게 행동하는 것이 바람직한 것인가를 가르쳐 줍니다. <br class="b">SM그룹은 경영윤리교육 프로그램을 통해 구성원들의 윤리의식을 고취시키고 도덕적 가치에 <br class="b">기초를 두고 의사결정하고 행동하도록 하고 있습니다. </p>
						</div>
					</div>
					<div class="sec_inner2 sec_inner">
						<div class="inner">
				...
				<div class="sec3 section">
					...
					<div class="btn_area">
						<a href="#" target="_blank" class="btn_default download_btn">윤리경영 다운로드</a>
					</div>
				</div>
	...
	<script>
		...
		$('.slide_cont').on('init reInit afterChange', function(event, slick, currentSlide, nextSlide){
			//currentSlide is undefined on init -- set it to 0 in this case (currentSlide is 0 based)
			var i = (currentSlide ? currentSlide : 0) + 1;
			console.log(currentSlide);
			$(this).siblings(".slide_control").find(".current").text(i);
		});
		$('.slide_wrap').each(function(index,item){
			$(item).find(".all").text($(item).find(".slide").length);
			$(item).find(".slide_prev").on("click" , function(e){
				e.preventDefault();
				$(item).find(".slide_cont").slick('slickPrev');
			});
			$(item).find(".slide_next").on("click" , function(e){
				e.preventDefault();
				$(item).find(".slide_cont").slick('slickNext');
			});
		});
		...
		$triggerTarget.each(function(idx,item){
			...
			var scaleMotion1 = tween.fromTo($titImg, 1, {scale:1.2},{scale:1})
														.fromTo($enTit, 1, {x:100 , opacity:0},{x:0 ,opacity :1},0.3);
			var staggerMotion1 = tween.staggerFromTo($list.find("[data-role='m_elm']"), 0.4,{y:50, opacity:0},{y:0, opacity:1},0.1);
			
			/* stagger motion */
			new ScrollMagic.Scene({triggerElement: item , offset : "200px"})
