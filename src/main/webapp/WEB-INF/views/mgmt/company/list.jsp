...
<body class="nav-md">
								...
								<form name="frmSearch" action="list.do" class="form-inline text-right">
									...
									<div class="form-group">
										<select name="SECTIONS" class="form-control">
											...
											<option value="${entry.key}" ${entry.key==SECTIONS?'selected':''}>${entry.value}</option>
											</c:forEach>
										</select>
  									</div>
									<div class="form-group">
										...
										<c:choose>
											<c:when test="${fn:length(list) > 0}">
												...
												<tr>
													...
													<td>
														<a href="modify.do?IDX=${row.IDX }" data-idx="${row.IDX }">${row.NAME }</a>
													</td>
													...
													<td class="text-center">${row.STATUS } / ${row.STATUS_EN }</td>
												</tr>
												</c:forEach>
											</c:when>
											<c:otherwise>
	...
	function fn_openBoardWrite(obj){
	    ...
	    comSubmit.setUrl("/mgmt/company/write.do");
		if("${!empty PID}"=="true"){
	...
	function fn_openBoardDetail(obj){
	    var comSubmit = new ComSubmit();
	    comSubmit.setUrl("/mgmt/company/modify.do");
